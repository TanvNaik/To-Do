...
const updateUI = function (desc, status) {
  ...
  html =
    '<tr><td>%Task%</td><td class="secondTd"><img id = "%id%" src="images/%status%.png" alt="%status%"></td></tr>';
...
const completedTask = function (event) {
  ...
  tasks.set(
    document.getElementById(event.target.id).parentNode.previousSibling
      .textContent,
    "complete"
  );
...
addbtn.addEventListener("click", function (e) {
  const child = e.target.matches("#add");
  if (child) {
...
window.onload = function () {
  for (let i = 0; i < localStorage.length; i++) {
    if (
      localStorage.getItem(localStorage.key(i)) === "pending" ||
      localStorage.getItem(localStorage.key(i)) === "complete"
    ) {
